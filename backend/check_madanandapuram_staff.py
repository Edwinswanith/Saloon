"""
Script to check Madanandapuram branch staff and their passwords
"""
import os
import sys
from datetime import datetime, timezone
from mongoengine import connect, disconnect
from models import Staff, Branch
from utils.auth import hash_password, verify_password

def connect_to_mongodb():
    """Connect to MongoDB using the same logic as app.py"""
    try:
        MONGODB_URI = os.environ.get('MONGODB_URI', 'mongodb+srv://edwin:Edwin006@saloon.8fxk7vz.mongodb.net/?appName=Saloon')
        MONGODB_DB = 'Saloon_prod'
        
        base_uri = MONGODB_URI
        
        if '@' in base_uri:
            parts = base_uri.split('@')
            if len(parts) == 2:
                credentials = parts[0]
                host_and_params = parts[1]
                
                if '/' in host_and_params:
                    host = host_and_params.split('/')[0]
                    if '?' in host_and_params:
                        params = '?' + host_and_params.split('?', 1)[1]
                    else:
                        params = ''
                    base_uri = f"{credentials}@{host}{params}"
        
        if f'/{MONGODB_DB}' not in base_uri:
            if '?' in base_uri:
                base_uri = base_uri.replace('?', f'/{MONGODB_DB}?')
            else:
                base_uri = f"{base_uri}/{MONGODB_DB}"
        
        if 'retryWrites' not in base_uri:
            separator = '&' if '?' in base_uri else '?'
            base_uri = f"{base_uri}{separator}retryWrites=true&w=majority"
        
        connect(host=base_uri, alias='default', db=MONGODB_DB,
                connectTimeoutMS=30000,
                serverSelectionTimeoutMS=30000)
        
        print(f"[OK] Connected to MongoDB: {MONGODB_DB}")
        return True
    except Exception as e:
        print(f"[ERROR] Failed to connect to MongoDB: {e}")
        return False

def sanitize_branch_name(branch_name):
    """Sanitize branch name for password generation"""
    if not branch_name:
        return 'Unknown'
    # Remove spaces and special characters, keep alphanumeric
    sanitized = ''.join(c for c in branch_name if c.isalnum())
    return sanitized if sanitized else 'Unknown'

def check_staff_passwords():
    """Check Madanandapuram branch staff and their passwords"""
    try:
        # Find Madanandapuram branch
        branch = Branch.objects(name='Madanandapuram').first()
        if not branch:
            print("[ERROR] Madanandapuram branch not found!")
            return False
        
        print(f"[OK] Found branch: {branch.name}")
        print(f"     Branch ID: {branch.id}")
        
        # Find all staff for this branch
        staff_list = Staff.objects(branch=branch).order_by('first_name')
        staff_count = staff_list.count()
        
        if staff_count == 0:
            print("[WARNING] No staff members found for Madanandapuram branch!")
            return False
        
        print(f"\n[OK] Found {staff_count} staff member(s) for Madanandapuram branch")
        print("=" * 80)
        
        # Sanitize branch name for password pattern
        branch_name_sanitized = sanitize_branch_name(branch.name)
        
        # Common password patterns to test
        test_passwords = []
        
        # Generate expected passwords based on pattern: staff{BranchName}{Number}
        for idx in range(1, staff_count + 1):
            test_passwords.append(f"staff{branch_name_sanitized}{idx}")
        
        # Also test common passwords
        test_passwords.extend([
            "staff123",
            "staff1",
            "password",
            "123456"
        ])
        
        print("\nSTAFF MEMBERS AND PASSWORDS:")
        print("=" * 80)
        
        for idx, staff in enumerate(staff_list, 1):
            full_name = f"{staff.first_name} {staff.last_name or ''}".strip()
            print(f"\n{idx}. {full_name}")
            print(f"   Mobile: {staff.mobile}")
            print(f"   Email: {staff.email or 'N/A'}")
            print(f"   Status: {staff.status}")
            print(f"   Password Hash: {staff.password_hash[:30]}..." if staff.password_hash else "   Password Hash: NOT SET")
            
            # Try to find matching password
            password_found = False
            if staff.password_hash:
                for test_pwd in test_passwords:
                    if verify_password(test_pwd, staff.password_hash):
                        print(f"   [OK] PASSWORD: {test_pwd}")
                        password_found = True
                        break
                
                if not password_found:
                    print(f"   [WARNING] Password not found in common patterns")
                    print(f"   Expected pattern: staff{branch_name_sanitized}{idx}")
            else:
                print(f"   [WARNING] No password hash set")
                print(f"   Expected password: staff{branch_name_sanitized}{idx}")
        
        print("\n" + "=" * 80)
        print("LOGIN CREDENTIALS SUMMARY:")
        print("=" * 80)
        
        for idx, staff in enumerate(staff_list, 1):
            full_name = f"{staff.first_name} {staff.last_name or ''}".strip()
            expected_password = f"staff{branch_name_sanitized}{idx}"
            
            print(f"\nStaff #{idx}: {full_name}")
            print(f"  Mobile: {staff.mobile}")
            
            if staff.password_hash:
                # Find actual password
                actual_password = None
                for test_pwd in test_passwords:
                    if verify_password(test_pwd, staff.password_hash):
                        actual_password = test_pwd
                        break
                
                if actual_password:
                    print(f"  Password: {actual_password}")
                else:
                    print(f"  Password: {expected_password} (expected, but verification failed)")
            else:
                print(f"  Password: NOT SET")
        
        return True
        
    except Exception as e:
        print(f"\n[ERROR] Failed to check staff passwords: {e}")
        import traceback
        traceback.print_exc()
        return False

def main():
    """Main function"""
    print("=" * 80)
    print("MADANANDAPURAM BRANCH STAFF PASSWORD CHECK")
    print("=" * 80)
    
    if not connect_to_mongodb():
        sys.exit(1)
    
    try:
        success = check_staff_passwords()
        if not success:
            sys.exit(1)
    finally:
        disconnect()
        print("\n[OK] Disconnected from MongoDB")

if __name__ == '__main__':
    main()


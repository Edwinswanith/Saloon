"""List every massage transaction with full customer + branch detail."""
import os
...
# 1) Services containing "massage"
massage_services = list(db.services.find(
    {'name': {'$regex': 'massage', '$options': 'i'}},
    {'_id': 1, 'name': 1}
))
massage_service_ids = {s['_id'] for s in massage_services}
service_name_map = {s['_id']: s.get('name', '') for s in massage_services}

# 2) Pull bills with at least one massage-related item
or_clauses = [{'items.name': {'$regex': 'massage', '$options': 'i'}}]
if massage_service_ids:
    or_clauses.append({'items.service': {'$in': list(massage_service_ids)}})
...
# 3) Build massage transaction rows + collect ref ids for batch lookup
...
for bill in bills:
    ...
    for item in bill.get('items', []):
        ...
        if 'massage' not in name and svc_ref not in massage_service_ids:
            continue
        staff_ref = item.get('staff')
        if staff_ref:
            staff_ids.add(staff_ref)
        transactions.append({
            'bill_date': bdate,
            'bill_number': bnum,
            'payment_mode': pay,
            ...
            'branch_id': brid,
            'staff_id': staff_ref,
            'service_name': item.get('name') or service_name_map.get(svc_ref, '<unknown>'),
...
# 4) Batch-fetch customers, branches, staff
...
print(f"Massage services in catalog : {len(massage_services)}")
print(f"Bills with massage items    : {len(bills)}")
print(f"Massage line-items total    : {len(transactions)}")
print(f"Distinct customers on bills : {len(customer_ids)}\n")
print("=" * 100)

# 6) Per-transaction detail
